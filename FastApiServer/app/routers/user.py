...
async def register_user(dto: UserDTO, db: Session = Depends(get_db)):
    ...
    print("a")
    user_id = user_crud.find_userid_by_email(request_user=dto)
    print("b")
    if user_id == "":
...
async def login(dto: UserDTO, db: Session = Depends(get_db)):
    ...
    user_id = user_crud.find_userid_by_email(request_user=dto)
    dto.user_id = user_id
    print(f"로그인 확인 id : {dto.user_id}, ps : {dto.password}")
    if user_id != "":
        login_user = user_crud.login(request_user=dto)
        if login_user is not None:
            print(f"로그인 성공정보: {login_user}")
            new_token = generate_token(login.user_email)
            login_user.token = new_token
            result = {"data":login_user}
        else:
            print(f"로그인 실패")
            result = JSONResponse(status_code=400, content=dict(msg="비번 틀림"))
    else:
        result = JSONResponse(status_code=400, content=dict(msg="이메일 틀림"))
    return result
